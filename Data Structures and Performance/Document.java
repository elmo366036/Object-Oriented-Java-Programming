...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		int vowelCount = 0; //without repeats
		String vowels = "aeiouyAEIOUY";
		boolean vowelFlag = false;		
		char[] wordArray = word.toCharArray();
		//iterate through wordArray. count vowels but not vowels immediately after a vowel
		for (int i = 0; i < wordArray.length; i++) {
			String currVowel = wordArray[i]+"";
			//if char is a vowel and flag not set (i.e., first vowel in a series of vowels)
			if (vowels.contains(currVowel) && (!vowelFlag)) {
				vowelFlag = true;				
				vowelCount++;					
			}
			else if (!vowels.contains(currVowel)) {
				vowelFlag = false;
			}
		}	
		numOfSyllables = vowelCount;
		//address e at end of word	
		if (wordArray[wordArray.length - 1] == 'e') { 
			//if one vowelCount or less, set numOfSyllables to 1
			if (vowelCount <= 1) {
				numOfSyllables = 1;
			}
			else {
				//address segue -> ue at end (vowel + e) at end
				//if second to last letter isn't a vowel, set numOfSyllables to vowelCount-1
				if (!vowels.contains(wordArray[word.toCharArray().length - 2]+"")) {
				numOfSyllables = vowelCount - 1;
				}
			}
		}
