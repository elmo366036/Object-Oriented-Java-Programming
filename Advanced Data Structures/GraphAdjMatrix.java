...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] matrixProduct = new int[adjMatrix.length][adjMatrix.length];			
		for (int i = 0; i < adjMatrix.length; i++) {
			for (int j = 0; j < adjMatrix.length; j++) {
				int dotProduct = 0;
				for (int k = 0; k < adjMatrix.length; k++) {
					dotProduct += adjMatrix[i][k]*adjMatrix[k][j];
				}
				matrixProduct[i][j] = dotProduct;
			}
		}
		for (int i = 0; i < matrixProduct.length; i++) {
			if (matrixProduct[v][i] > 0){
				for (int j = 1; j <= matrixProduct[v][i]; j++) {			
					twoDistance.add(i);	
				}
			}
		}			
